# -*- coding: utf-8 -*-
"""生成 WB_2026-09-09_硬件看板.html（每日智能终端硬件情报日报，单 HTML / 内联 CSS / 无 CDN）"""
...
cn = json.load(open(f"{BASE}/_agent_cn_2026-09-09.json", encoding="utf-8"))
intl = json.load(open(f"{BASE}/_agent_intl_2026-09-09.json", encoding="utf-8"))
...
summary_rows = "\n".join(summary_row(it) for it in all_items)
cn_cards = "\n".join(card_html(it) for it in cn_sorted)
intl_cards = "\n".join(card_html(it) for it in intl_sorted)
...
out = f"{BASE}/WB_{DATE}_硬件看板.html"
...
print("国内首条卡:", cn_sorted[0]["title"], "| 国际首条卡:", intl_sorted[0]["title"])
